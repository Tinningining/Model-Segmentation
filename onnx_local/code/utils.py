...
def load_tokenizer(tokenizer_dir: str):
    ...
    try:
        ...
        tokenizer_path = str(Path(tokenizer_dir).resolve())
...
def pad_input_ids(
    ...
    Args:
        input_ids: shape (1, q_len), dtype=int64
        max_input_len: 目标长度
        pad_id: 填充 ID

    Returns:
        padded: shape (1, max_input_len), dtype=float32
    """
    q_len = input_ids.shape[1]
    if q_len >= max_input_len:
        return input_ids[:, :max_input_len].astype(np.float32)
