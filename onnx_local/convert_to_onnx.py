...
class Qwen3BlockStackNoPastWrapper(nn.Module):
    """Block 包装器：无 past_key/past_value 输入，用于 system 阶段。"""
...
def export_system_onnx(base, onnx_path: Path, system_len: int):
    """导出 System 阶段 ONNX 模型（处理固定 system prompt，无 past KV）。

    与原 prefill 结构相同：seq_len=system_len，无 past_key/past_value。
    用于预计算固定 system prompt 的 KV cache。
    """
    cfg = base.config

    # 1) Embedding
    emb = Qwen3EmbeddingModule(base).eval()
    ids = torch.zeros(1, system_len, dtype=torch.float32)
    torch.onnx.export(
        emb,
        (ids,),
    ...
    # 2) Blocks（无 past_key/past_value，system 首次处理）
    blocks = [
        ("layers_0_6", 0, 7),
        ("layers_7_13", 7, 14),
        ("layers_14_20", 14, 21),
    ...
    for name, s, e in blocks:
        ...
        wrapper = Qwen3BlockStackNoPastWrapper(blk).eval()
        hs = torch.zeros(1, system_len, cfg.hidden_size)
        attn = torch.zeros(1, 1, system_len, system_len)
        pos = torch.arange(system_len, dtype=torch.long).unsqueeze(0)
        torch.onnx.export(
            wrapper,
            (hs, attn, pos),
    ...
    # 3) Output（system 阶段不需要 output，但为完整性保留）
    ...
    print(f"Exported System ONNX to {onnx_path.resolve()}")


def export_prefill_onnx(base, onnx_path: Path, prefill_len: int, max_cache_len: int):
    """导出 Prefill 阶段 ONNX 模型（处理用户输入，带 past KV from system stage）。

    seq_len=prefill_len，带 past_key/past_value。
    past KV 来自 system 阶段预计算的 KV cache。
    """
    cfg = base.config
    ...
    # 2) Blocks（带 past_key/past_value，来自 system 阶段的 KV cache）
    blocks = [
        ("layers_0_6", 0, 7),
        ("layers_7_13", 7, 14),
        ("layers_14_20", 14, 21),
    ...
    for name, s, e in blocks:
        ...
        # Prefill with past: attention mask 最后一维 = max_cache_len + prefill_len
        attn = torch.zeros(1, 1, prefill_len, max_cache_len + prefill_len)
        pos = torch.zeros(1, prefill_len, dtype=torch.long)
...
def export_decode_onnx(base, onnx_path: Path, max_cache_len: int):
    """导出 Decode 阶段 ONNX 模型（seq_len=1，带 past KV）。不变。"""
    cfg = base.config
    kv_heads = cfg.num_key_value_heads
    head_dim = cfg.hidden_size // cfg.num_attention_heads
    decode_len = 1
...
def export_onnx(model_path: str, onnx_dir: str, system_len: int = 256,
                prefill_len: int = 512, max_cache_len: int = 1024):
    """导出 System、Prefill 和 Decode 三组 ONNX 模型。"""
    base = load_base_qwen3(model_path)
    cfg_dict = base.config.to_dict()

    # System 组（处理固定 system prompt，无 past KV）
    system_path = Path(onnx_dir) / "system"
    system_path.mkdir(parents=True, exist_ok=True)
    export_system_onnx(base, system_path, system_len)
    with open(system_path / "config.json", "w", encoding="utf-8") as fw:
        json.dump(cfg_dict, fw, indent=2)

    # Prefill 组（处理用户输入，带 past KV from system）
    prefill_path = Path(onnx_dir) / "prefill"
    prefill_path.mkdir(parents=True, exist_ok=True)
    export_prefill_onnx(base, prefill_path, prefill_len, max_cache_len)
    with open(prefill_path / "config.json", "w", encoding="utf-8") as fw:
        json.dump(cfg_dict, fw, indent=2)

    # Decode 组（逐 token 生成，带 past KV）
    ...
    print(f"  System:  {system_path.resolve()} (seq_len={system_len})")
    print(f"  Prefill: {prefill_path.resolve()} (seq_len={prefill_len}, with past KV)")
    print(f"  Decode:  {decode_path.resolve()} (seq_len=1, max_cache={max_cache_len})")


if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("--model_path", default=r"D:\qwen_split\qwen3_1.7b")
    ap.add_argument("--onnx_dir", default="./onnx_models")
    ap.add_argument("--system_len", type=int, default=256)
    ap.add_argument("--prefill_len", type=int, default=512)
    ap.add_argument("--max_cache_len", type=int, default=1024)
    args = ap.parse_args()
    export_onnx(args.model_path, args.onnx_dir, args.system_len, args.prefill_len, args.max_cache_len)
